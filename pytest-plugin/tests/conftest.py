"""
Test infrastructure for Big Dill plugin tests.

Adds the plugin source to sys.path so that pytester-spawned sessions can import
pytest_big_dill without requiring a full editable install.  This mirrors what
...
# so that `pytest_plugins = ["pytest_big_dill.hooks"]` works in pytester conftest files.
